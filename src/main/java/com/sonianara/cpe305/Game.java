...
public class Game {
	...
	public boolean isHorizontal(ArrayList<Location> locArray) {
		for (int i = 0; i < locArray.size() - 1; i++) {
			if (locArray.get(i).getX() == (locArray.get(i + 1).getX() - 1) || (locArray.get(i).getX() == (locArray.get(i + 1).getX() + 1))) {
	...
	public boolean isVertical(ArrayList<Location> locArray) {
		for (int i = 0; i < locArray.size() - 1; i++) {
			if (locArray.get(i).getX() == locArray.get(i + 1).getX()) {
				if (locArray.get(i).getY() == (locArray.get(i + 1).getY() - 1) || (locArray.get(i).getY() == (locArray.get(i + 1).getY() + 1))) {
					return true;
				}
			}
		}
	...
	public ArrayList<Location> traverseRightCoordinates(ArrayList<Location> arrLoc, Board wholeBoard) {
	...
	public ArrayList<Location> traverseUpperCoordinates(ArrayList<Location> arrLoc, Board wholeBoard) {
	...
	public ArrayList<Location> traverseLowerCoordinates(ArrayList<Location> arrLoc, Board wholeBoard) {
  ...
  //if the word is horizontal, get the coordinate of the right-most letter 
	...
	public boolean isAdjacentToAWord(Board previousGameBoard, ArrayList<Location> coordinates) {
		...
		for (int i = 0; i < coordinates.size(); i++) {
			if (isHorizontal(coordinates)) {
				...
				if (bordersTop(previousGameBoard, coordinates.get(i)) || bordersBottom(previousGameBoard, coordinates.get(i))) {
					if (bordersLeft(previousGameBoard, leftest) || bordersRight(previousGameBoard, rightest)) {
						isAdjacent = true;
					}
				}
			}
				...
				if (bordersLeft(previousGameBoard, coordinates.get(i)) || bordersRight(previousGameBoard, coordinates.get(i))) {
					if (bordersTop(previousGameBoard, topmost) || bordersBottom(previousGameBoard, bottomost)) {
						isAdjacent = true;
					}
				}
			}
	...
	public boolean isValid(Player p, ArrayList<Location> coordinates, Board previousGameBoard) {
		if (playedFromRack(p, coordinates, previousGameBoard) && isAdjacentToAWord(previousGameBoard, coordinates) && checkWord(coordinates, previousGameBoard)) {
			return true;
		}
		return false;
	}
	
	public boolean isValidMovePlayerOne(Player p, ArrayList<Location> coordinates, Board previousGameBoard) {
		if (playedFromRack(p, coordinates, previousGameBoard) && checkWord(coordinates, previousGameBoard)) {
			return true;
		}
		return false;
	}
